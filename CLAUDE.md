...
**`fonts/` is staged, not wired.** `62968c7` committed `fonts/space-grotesk-latin-var.woff2` and `fonts/jetbrains-mono-latin-var.woff2` for a future switch to self-hosting, and nothing references them yet: all three pages still `preconnect` + `preload` the same Google Fonts stylesheet, and there is no `@font-face` anywhere. Only `configs/nginx.conf` knows the extension, in its blanket no-cache rule. If you do the switch, remember the directory is invisible to both shipping mechanisms: `deploy.yml` copies `images cli scheme` by name and `release.yml` zips a named list, so `fonts/` reaches the container (blanket `COPY . .`) and nothing else. It would work locally on `:8080` and 404 in production.
...
Internal docs (`CLAUDE.md` anywhere, `scheme/tools/`, `scheme/docs/`) never reach production.

**The root review working documents are a deliberate exception.** `SCHEME-REVIEW.md`, `SCHEME-2.4-PLAN.md` and `SCHEME-2.4-STAGE-K.md` were dropped from `.dockerignore` in `2f8da46`, so the local container image contains them and serves them at 200. That is fine, because the container is a local iteration tool and the two shipping mechanisms are allowlists: an untracked file at the repo root cannot reach GitHub Pages or the release zip whether or not anything names it. **Do not "fix" that 200 by re-adding the pattern.** It was decided once with reasons written into the commit message, and undone once on 2026-07-30 by a later pass that read the 200 as a leak. If the container itself ever ships, that is the moment the pattern has to come back. As of 2026-08-01 none of those three files is on disk any more (the review that produced them closed), so nothing is being served at 200 today: the paragraph stays because the decision is about the NEXT root working document, not about those filenames. Three mechanisms have to agree: `deploy.yml` (GitHub Pages), `release.yml` (the zip) and `.dockerignore` (the container image). The first two are allowlists, so a new internal file at the repo root needs only `.dockerignore`; anything inside an already-copied directory (`scheme/`, `cli/`, `images/`) must be named in all three. The local container is the easiest place to catch a miss: `curl -s -o /dev/null -w '%{http_code}' http://localhost:8080/<path>` must return 404. Any push to `main` ships immediately; there is no staging environment. Hosting is GitHub Pages + Cloudflare (custom domain, SSL, edge cache).
...
Two numbers drift faster than anything else and are worth grepping whenever the harness changes: **how many checks `npm run gate` chains** (stated in `README.md` and twice in `scheme/CLAUDE.md`, and wrong in all three at some point) and **the per-card design notes in `scheme/docs/CARDS.md`**, which anchor themselves to a line of code and rot silently when that line moves. The second is now machine-checked by `check-notes` in the gate; the first is not, so it is on you.
