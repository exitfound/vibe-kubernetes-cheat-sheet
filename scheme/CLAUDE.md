...
- `check-notes.mjs`: does every design note still point at code that exists? `docs/CARDS.md` anchors each note to the line it sat above (`### before `<code>``) and **nothing had ever verified those anchors**, so a rename, a codemod or a relayout silently turns a geometry record into a note about a line that is gone. It checks three things: every anchor still occurs in its card, every `## <id>` section has a file, and every card has a section. No browser and no server, so it is the second link in the chain. **In the gate.** Written 2026-07-27; the catalog is at 608 anchors, 0 stale, and **no card is without a design record any more** (`cluster-admission-webhooks`, `cluster-etcd-raft` and `cluster-leader-election` were the last three and got theirs during review stage 2.4).
