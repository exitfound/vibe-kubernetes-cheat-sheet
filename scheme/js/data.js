...
export const SCHEMES = [
  ...
  },
  {
    ...
    category: 'cluster',
    subcategory: 'control-plane',
    desc: 'What actually happens between kubectl apply and a running Pod? The manifest travels to the API and lands in ETCD, and then a chain of watchers takes over: the Deployment controller creates a ReplicaSet, the ReplicaSet controller creates a Pod, the scheduler assigns that Pod a Node, and the Kubelet there starts the container. Every handoff is one component reacting to a change on its own watch rather than a call from the component before it.',
    k8sVersion: '1.35',
  ...
  },
  {
    ...
    category: 'cluster',
    subcategory: 'worker-nodes',
    desc: 'A container blows past its memory limit, so what kills it and what gets recorded? The limit is enforced by the kernel cgroup, and once usage reaches the cap and reclaim cannot free enough, the cgroup out-of-memory killer sends SIGKILL, ending the process with exit code 137. The Kubelet only witnesses this. It records the OOMKilled reason in the container status, then restarts the container according to the Pod restart policy.',
    k8sVersion: '1.35',
