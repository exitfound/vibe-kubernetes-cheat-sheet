...
import { valChip, setVal, pulsePod, clearPodHighlight, connectorPacket, makeInit } from '../lib/scheme-kit.js';
...
class Scene {
  ...
  build() {
    ...
    const apiserver = box({ x: 580, y: 40, w: 220, h: 80, label: 'ApiServer', sublabel: 'PriorityClass + delete + bind', cat: 'control' });
...
function clearHL(s) {
  ...
  ['pod1','pod2','pod3','podNew'].forEach(k => clearPodHighlight(s.refs[k]));
}
...
const STEPS = [
  ...
  },
  {
    ...
    enter(s, ctx) {
      ...
      // Pod A pulses and fades out only when the DELETE reaches the node (~1700ms).
      pulsePod(s.refs.pod1, ctx, 1700);
  ...
  },
  {
    ...
    enter(s, ctx) {
      ...
      ctx.register(animateAlong(pCon, [[320, 80], [280, 80], [280, 550], [320, 550]], { duration: 1100, delay: 600 }));
      ctx.register(pCon.animate([{ opacity: 1 }, { opacity: 0 }], { duration: 200, delay: 1700, fill: 'forwards', easing: 'ease-in' }));
      // Pod NEW pulses once (pulse fades) and materializes in the freed slot when the bind reaches the node (~1700ms).
      pulsePod(s.refs.podNew, ctx, 1700);
      ctx.register(s.refs.podNew.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 800, delay: 1700, fill: 'both', easing: 'ease-out' }));
    },
  },
];
