...
//
// WHY EACH ANIMATION CARRIES AN `onfinish` FLAG. A frozen probe never fires a completion handler
// (III.5: enterStep pauses, seekStep moves paused time), so everything that lives in one used to be
// invisible to all three halves of the oracle. That is not a corner: `at()` and `lightBoxAt()` hang
// their entire effect off a handler, and so does the declarative `F.fade({ unlight })`. Measured on
// this catalogue, both directions: DELETING `unlight` from cluster-node-drain still dumped
// CARD CLEAN, and ADDING an `unlight` to cluster-graceful-node-shutdown, which carries none by
// design, left all three halves clean. Two agents hit this independently and each hand-rolled a
// private probe for it, which is the signal it belongs in the tool.
// The flag is the FACT of a handler, never its body. A serialised body would redden on a rename
// inside the closure and turn the oracle noisy, which is the failure mode that kills an oracle.
// What it still cannot see: WHICH handler it is, and any handler attached by
// addEventListener('finish') rather than the property (nothing in scheme/js does that today).
  ...
  return page.evaluate(({ sel, offsets }) => {
    ...
    const meta = anims.map(a => {
      ...
      return {
        ...
        // Read HERE, before the sample loop below seeks: the fact alone, see the file header.
        // Pausing keeps the property attached either way, so the position is belt and braces.
        onfinish: !!a.onfinish,
        samples: [],
      };
    });
...
function renderTable(id, steps, offsets) {
  ...
  for (const s of steps) {
    ...
    out.push('   ' + padR('target', w) + '  ' + padR('prop', 14) + padR('dur/delay', 11) + padR('easing', 11) + padR('fin', 5) + pct.map(p => padL(p, SW)).join(''));
    for (const a of data.anims) {
      const timing = `${a.durMs}/${a.delayMs}`;
      out.push('   ' + padR(a.target, w) + '  ' + padR(a.primary || a.props[0] || '', 14) + padR(timing, 11) + padR(String(a.easing), 11) + padR(a.onfinish ? 'fin' : '', 5) + a.samples.map(v => padL(v, SW)).join(''));
    }
  }
  return out.join('\n');
}
