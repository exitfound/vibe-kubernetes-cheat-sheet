...
- **The harness**: what each test catches and what it is BLIND to. That lives in the HEADER of the
  ...
  is `scheme/test/package.json`, where it executes. The migration oracle (`anim-dump`, `dom-dump`)
  is `scheme/test/oracle/` and goes away with the card migration it exists for.
...
| `Source` | where the long form, the measurement or the implementation lives. **A file, never a line number**: nothing checks this column, and on 2026-08-07 fifteen of the sixteen citations into the old `check-canon.mjs` missed their line by 9 to 12 and landed in a comment. Rewritten on 2026-08-07 off the deleted script harness onto the test, the code file or the measurement that holds each answer now. The `Check` column is the anchor that greps, and unlike this one it is checked |
...
| M-14 | Every packet ripples at its destination. The delivered cue is part of the arrival canon with no per-call opt-in, and the ripple carries `.scheme-ripple` rather than `.scheme-packet` so anything counting packets sees ONE ball per hop | test:motion/ARRIVE | `lib/scheme-kit.js`, `arrivalRipple` |
...
| M-20 | **Geometry changes are timing changes.** After ANY geometry change re-run `anim-dump` on every step, not just the one you moved | test:duration/M-20 | `test/render/duration.test.mjs` |
...
| C-02 | **Pass `role` explicitly, always.** The kit defaults it to nothing (`role \|\| null`) and a tinted dialog collapses most roles onto the one tint, so a wrong role is usually invisible | test:palette/UNPAINTED | `lib/scheme-kit.js`, `valChip` |
| C-03 | One `(category, element class, role, state)` tuple resolves to ONE colour. `check-palette` catches a role that resolves inconsistently, NEVER a role that was the wrong one to ask for | test:palette/SPREAD, report:palette-steps/CONFLICTING | `test/render/palette.test.mjs` |
...
| C-12 | `check-opacity` judges the EXPRESSION, not the number, so a named constant cannot smuggle a shade past it | test:opacity/PHASE | `test/render/opacity.test.mjs` |
...
| C-20 | `color-mix` is deliberately unused, so colour resolution stays fully deterministic | review | `scheme/css/styles.css`, the tinted dialog block |
...
| T-08 | `check-terms` reads `desc`, every `narration` and every `aria-label`. It cannot read MEANING | test:inline/T-08 | `test/render/inline.test.mjs` |
...
| T-28 | A card's `aria-label` is a FULL SENTENCE describing the whole drawing | test:inline/T-28 | `scheme/CLAUDE.md` |
| T-29 | Wire labels are dim `text` at fixed positions, blank (`' '`) at build, filled per step with `setWire` | report:reduced/WIRE-TEXT | `lib/scheme-kit.js` |
| T-30 | A card whose `setWire` runs only below the `ctx.reduced` guard shows blank lanes on prev and reset while the narration names the exact string that should be there | report:reduced/WIRE-TEXT | `test/render/reduced.test.mjs` |
...
| ID | Rule | Check | Source |
|---|---|---|---|
| P-01 | **Every step states EVERY chip**, not only the ones it narrates: a legacy card through ONE `setChips(s, {...})` call, a migrated card through its `chips` field. An unset chip keeps the previous step's value and silently lies | test:spec-steps/P-01 | this file. The CONVENTION is machine-checked on every migrated card, whether a carried-over value is still TRUE stays a human's job |
| P-01a | **The 2026-08-06 fold made the wrapper the norm, it did NOT close the rule.** 101 of 108 cards carry the wrapper, the other 7 hold no chip or exactly one and state it directly. The claim of 0 unstated steps was withdrawn on 2026-08-07 and is TRUE AGAIN FOR CLUSTER ONLY: the three steps of `cluster-scheduler-decision` that carried verdicts over (`score` from `filter`, then `bind` and `placed`) write all 10 of them out since the migration, and reading the 21 cluster cards as data now gives **0 chip-set differences over 137 steps**. Workloads, network and storage are still legacy and still unmeasured | review, test:spec-steps/P-01 | re-measured 2026-08-07 over cluster. The value each of the three carried over was TRUE as drawn, so writing it out was a same-picture fix |
| P-11 | The wrapper is a LINE-INITIAL `function setChips(s, { ... })` declaration with a DESTRUCTURED second parameter, and each call passes an OBJECT LITERAL. Any other form (an arrow, a method, an `Object.entries` loop, a spread at the call site) is invisible to `prose.mjs`. See T-16 | review | `test/fixtures/prose.mjs` |
...
| P-03 | **A chip must not run ahead of the motion that produces its value.** Pin the end value above the guard, then on the played path set the chip back to what the step STARTS from and turn it over on `pkt.arrivalMs` through `at(...)` | report:arrival/R2 | `test/report/arrival.test.mjs`. Since the chip fold, that start value is written out at the call site, so the pattern reads off the step |
...
| P-10 | `setChip` must not be renamed without renaming its seed in `prose.mjs`. See T-16 | review | `lib/scheme-kit.js`, `setChip` |
...
| ID | Rule | Check | Source |
|---|---|---|---|
| S-01 | `class Scene { constructor(host){ this.host=host; this.refs={}; this.build(); } build(){...} reset(){ this.build(); } }`, exactly once per card | test:skeleton/S-01 | `test/unit/skeleton.test.mjs` |
| S-02 | **A card module has exactly TWO legal export surfaces and nothing between them**: legacy is `init` alone, from `export const init = makeInit(Scene, STEPS, { posterFirst: true });`, and migrated is `SCENE`, `STEPS_SPEC` and `init`, from `defineCard(SCENE, STEPS_SPEC, { posterFirst: true })`. The comparison is set EQUALITY per card, never containment, and the split between the two forms IS the migration counter | test:module/S-02 | `test/unit/module.test.mjs` |
...
| S-10 | **Every `enter()` opens with `resetStep(s);` and nothing before it** | test:skeleton/S-10 | `test/unit/skeleton.test.mjs` |
| S-11 | `resetStep(s)` is declared once per card as a line-initial `function`, and its body is fixed in shape: `s.refs.packetLayer.replaceChildren()` FIRST, then the card's own `clearHighlights(...)` and extras, then `clearWires(s)` LAST. Only the middle varies, which is why it stays per card. **The generated prologue disagrees on that order and it is unnoticed rather than fine**: `makeResetStep` runs `reset.extra` AFTER `clearWires`, invisible today only because the one extra in the catalog touches `strokeDasharray` and nothing a wire label owns | test:skeleton/S-11 | `test/unit/skeleton.test.mjs` |
...
| S-13 | After `resetStep` a step sets ALL chip values, wire labels and `.highlight` classes, and **pins final opacities inline**, so a cancel mid-step lands on the right value | test:reduced/OPACITY-OWN | `scheme/CLAUDE.md` |
| S-14 | **The reduced-motion split is the load-bearing line.** Everything ABOVE `if (ctx.reduced) return;` is the complete static end-state; everything BELOW is motion | test:reduced/OPACITY-OWN | `test/render/reduced.test.mjs` |
| S-15 | **Never animate state that is not also pinned statically above the guard** | test:reduced/OPACITY-OWN | `test/render/reduced.test.mjs` |
| S-16 | `check-reduced` compares four axes: own opacity, INHERITED (effective) opacity multiplied down the ancestor chain, drawn wire text, and the `.highlight` set | test:reduced/OPACITY-OWN | `test/render/reduced.test.mjs` |
| S-17 | Whatever lights on ARRIVAL must also light on the REDUCED path, so the class goes in the guard body: `if (ctx.reduced) { s.refs.api.classList.add('highlight'); return; }` above, `lightBoxAt(s.refs.api, ctx, pkt.arrivalMs)` below | test:reduced/OPACITY-OWN | `lib/scheme-kit.js`, `lightBoxAt` |
...
| S-19 | A `.highlight` put on a Pod INNER BOX has to be cleared BY NAME in `clearHighlights`'s keys list. The `pods` argument runs `clearPodHighlight`, which resets inline stroke styles and touches NO class. **Confusing the two leaks a class that ACCUMULATES**: prev and reset replay 0..n, so the box stays lit for the rest of the card and gathers one more with every step, which is how FIVE networking cards carried it at once. Nothing in the suite can see it: `render/reduced.test.mjs` compares the two paths against each other and both accumulate identically (its `HIGHLIGHT` axis reports rather than fails in any case), arrival looks at receivers, and palette reads a lit element as a legal state. Only a per-step dump of the whole class set shows it | review | `lib/scheme-kit.js`, `clearHighlights` |
| S-20 | **A folder holds exactly four kinds of `.js`**: its cards, its `<category>-kit.js`, its `cards.js`, its `posters.js` | test:module/S-20 | `test/unit/module.test.mjs` |
| S-21 | **A card imports its own kit and nothing past it**: `../../lib/svg.js`, `../../lib/primitives.js`, `./<category>-kit.js`. `lib/` holds only what every category shares | test:module/S-21 | `scheme/CLAUDE.md`, listed under `check-canon`'s blind spots |
| S-22 | The four kits re-export the SAME list of names from `scheme-kit`. **The size of that list is not written down anywhere on purpose: `R-kitparity` is the source of truth.** It compares the four to each other, so a name added to one and not the rest fails immediately | test:module/S-22 | `test/unit/module.test.mjs` |
...
| S-25 | `flashChips` is in the kit surface with zero callers, and **the decision to delete it was taken and then REVERSED**: no calls is not proof of dead code, and this export stays. Do not call it, and do not drop it from one kit, because the four re-export lists only move together | test:module/S-22 | `lib/scheme-kit.js`, `flashChips` |
...
| S-36 | Each card carries exactly ONE pointer comment under its imports: `// Design notes for this card: ./CARDS.md#<id>` | review | `test/unit/docs.test.mjs` |
...
| S-38 | **A note anchor is DATA: never reword one.** `check-notes` anchors each note to a line of code with ``### before `<line>` ``. **124 anchors today**, all four `CARDS.md` (cluster 15, workloads 24, network 41, storage 44). The 169 this row used to claim matched nothing. **61 more anchors lived in a record that was deleted, and the old walk skipped a file it could not open SILENTLY at exit 0** (`if (!existsSync(md)) continue;`), so the count fell 185 to 124 with no finding and no error. `unit/docs.test.mjs` fails on a record it cannot read instead of running shorter | test:docs/A2 | `test/unit/docs.test.mjs`, counted 2026-08-07 |
...
| S-47 | **This file has to tell the truth about itself.** A row claiming `test:<file>/<name>` names a file `npm test` really runs and a name that file really prints, no id is used twice, no id block skips a number, and every test file is cited by at least one rule. Otherwise the rulebook drifts from the harness the way one number once drifted across six files | test:docs/E1 | `test/unit/docs.test.mjs`, group E |
...
| `STO.S-01` | what `setStage` / `setBorn` pins |
...
## Rules the declarative refactor will make FALSE

A working list, not a change. Every rule below is true today and must be obeyed today: this section
only records which ones stop being true once a card is described by data (`SCENE` / `STEPS_SPEC`),
once the script harness is replaced by `scheme/test/`, and once the deleted design record stays
deleted. **Do not edit, weaken or delete any of them ahead of that work**: a rule retired before its subject is gone
leaves a live defect unwritten, and this is the file that is supposed to tell the truth about itself
(`S-47`). Taken from the 2026-08-07 inventory of all 235 rows.

Written as a list rather than a table on purpose: a row here is a POINTER to a rule, and a table row
starting with an id is what `check-canonrows` counts as the rule itself.

- **M-35** names `frame-strip`, a reader deleted with the script harness and with no successor. Only the mechanism survives, in the seek helper the render fixture carries.
- **C-02** `role` stops being passed at every call site: the category kit binds it once. See S-42.
- **T-15** indirect coverage is an artefact of scraping the source. A spec read by import has none.
- **T-16** both collapse modes it documents live in `prose.mjs`, which goes.
- **T-17** the class "a chip write the resolver could not read" cannot exist once the input is an import.
- **T-17a** it enumerates those 8 writes, so it goes with T-17.
- **T-30** a generated `enter` pins wire labels above the guard by construction, so that defect cannot be written.
- **T-34** it cites `check-sources`, which is deleted with no test successor. Either this row goes or the test plan is short one test.
- **P-10** the `prose.mjs` seed it protects stops existing.
- **P-11** the exact shape of the `setChips` wrapper is a requirement `prose.mjs` imposes and nothing else does.
- **P-15** the ban on a chip factory is lifted: a spec is read by import, not by seeding on a literal.
- **S-01** the `class Scene` literal stops being written per card.
- **S-03** `buildScene` opens the build, so no card writes that line.
- **S-05** it is a rule about a CHECK rather than about a card. The principle inside it (a rule that is silent when its subject disappears is not a rule) is process and belongs in the root `CLAUDE.md`.
- **S-10** there is no hand-written `enter()` left to open with `resetStep(s)`.
- **S-11** `resetStep` is generated from `SCENE.reset` instead of declared per card.
- **S-12** the same, it bans a declaration nothing will be in a position to write.
- **S-26** `defineCard` stops being rejected, which is the whole of the code work.
- **S-27** its own revisit condition (`build()` stops being per card) is met.

**Four came off this list on 2026-08-07, in the reword pass the last paragraph below names.** `D-10`,
`S-37`, `S-40` and `S-46` each named a deleted file rather than a superseded idea, so each was
rewritten onto its live subject and none was retired: the subcategory order beside the list it
orders, a note beside the code it describes, a test header, and a walk that may not skip what it
cannot open. Nineteen rows are left.

Three rules were expected here and do NOT belong. **`S-13` and `S-14` stay TRUE**: the static-first
split survives, it only moves from a line of source into the generated `enter`. **`S-25` stays
true** because the decision to delete `flashChips` was reversed, which is what `S-25` itself asks
for. One further class is deliberately not listed: a rule that stays true while NAMING a thing that
disappears (a tool, a file, a line of source) needs rewording rather than retirement, and that is
one pass at the end, not one row at a time.
